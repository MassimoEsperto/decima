...
$sql_ranking = "SELECT r.squadra_id as id,((AVG(r.goals)*3)+AVG(r.punti))*10 as ranking, ";
...
$sql_ranking .="WHERE (g.is_calcolata = 1 or g.id_giornata = 1) GROUP BY r.squadra_id ORDER BY ranking DESC ";
...
if($result = mysqli_query($con,$sql_ranking))
{
	...
	while($row = mysqli_fetch_assoc($result))
	{
		...
		$ranking_[$ele]['id_squadra'] = $row['id'];
        $ranking_[$ele]['id_utente'] = $row['id_utente'];
